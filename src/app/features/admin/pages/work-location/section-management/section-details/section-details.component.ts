...
import { SectionDetailsInfoComponent } from './section-details-info/section-details-info.component';
...
import { AuditsSectionComponent } from '../../../../../../shared/components/audits-section/audits-section.component';
...
@Component({
  selector: 'app-secton-details',
  standalone: true,
  imports: [
    CommonModule,
    TranslateModule,
    SectionDetailsInfoComponent,
    TableDataComponent,
    TaskContainerComponent,
    FormsModule,
    SectionTreeComponent,
    SectionQuestionsComponent,
    AuditsSectionComponent,
  ],
...
export class SectionDetailsComponent {
  ...
  constructor(
    ...
    private sectonService: SectionService,
  ...
  ngOnInit(): void {
    ...
    // If user is auditor, force the location tab
    if (this.myRole === 'auditor') {
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      if (this.sectionId) {
        this.loadFloorDetails(this.sectionId);
      } else {
  ...
  loadFloorDetails(id: number): void {
    this.isLoading = true;
    this.sectonService.getSectionById(id).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load secton details. Please try again later.'
          );
        }
      },
    });
  ...
  loadUsersByFloor(sectionId: number): void {
    // Don't load users if user is auditor
    if (this.myRole === 'auditor') return;
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this secton');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this secton');
      },
    });
  }

  loadTasks(): void {
    // Don't load tasks if user is auditor
    if (this.myRole === 'auditor' || !this.sectionId) return;
  ...
  loadShifts(): void {
    // Don't load shifts if user is auditor
    if (this.myRole === 'auditor' || !this.sectionId) return;
  ...
  loadAttendance(): void {
    // Don't load attendance if user is auditor
    if (this.myRole === 'auditor' || !this.sectionId) return;
  ...
  loadLeaves(): void {
    // Don't load leaves if user is auditor
    if (this.myRole === 'auditor' || !this.sectionId) return;
  ...
  selectTab(tab: string): void {
    // If user is auditor, only allow 'audits' tab
    if (this.myRole === 'auditor' && tab !== 'audits') {
      return; // Block any tab other than 'audits'
    }

    this.selectedTab = tab;

    // Load data based on tab
    if (tab === 'users' && this.sectionId) {
      this.loadUsersByFloor(this.sectionId);
    }
    if (tab === 'tasks' && this.sectionId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.sectionId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.sectionId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.sectionId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `secton_${this.sectionId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(
      `Floor Report: ${this.sectionData?.name || 'Unknown'}`,
      105,
      yPos,
      {
        align: 'center',
    ...
    doc.save(`secton_${this.sectionId}_report.pdf`);
  }
  ...
  onEditFloor(): void {
    if (!this.sectionId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-secton`,
      this.sectionId,
    ]);
  }

  async onDeleteFloor(): Promise<void> {
    if (!this.sectionId || !this.sectionData) return;

    const result = await Swal.fire({
      title: 'Delete Floor?',
      text: `Are you sure you want to delete "${this.sectionData.name}" secton? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.sectonService.softDeleteSection(this.sectionId).subscribe({
        next: () => {
          Swal.fire('Deleted!', 'Floor has been deleted.', 'success');
          this.router.navigate([
            `/${this.currentUserRole.toLowerCase()}/sectons`,
          ]);
        },
        error: (err) => {
          Swal.fire('Error!', 'Failed to delete secton.', 'error');
        },
      });
    }
  }
