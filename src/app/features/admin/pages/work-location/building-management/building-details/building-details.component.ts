...
import { BuildingDetailsInfoComponent } from './building-details-info/building-details-info.component';
import { BuildingTreeComponent } from './building-tree/building-tree.component';
...
export class BuildingDetailsComponent {
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.buildingId = id ? +id : null;
      if (this.buildingId) {
        this.loadBuildingDetails(this.buildingId);
      } else {
  ...
  loadBuildingDetails(id: number): void {
    this.isLoading = true;
    this.buildingService.getBuildingById(id).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load building details. Please try again later.'
          );
        }
      },
    });
  ...
  loadUsersByBuilding(buildingId: number): void {
    this.isLoading = true;
    const filters = {
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this building');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this building');
      },
    });
  }
  ...
  selectTab(tab: string): void {
    ...
    if (tab === 'users' && this.buildingId) {
      this.loadUsersByBuilding(this.buildingId);
    }
    if (tab === 'tasks' && this.buildingId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.buildingId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.buildingId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.buildingId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `building_${this.buildingId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(
      `Building Report: ${this.buildingData?.name || 'Unknown'}`,
      105,
      yPos,
      {
        align: 'center',
    ...
    doc.save(`building_${this.buildingId}_report.pdf`);
  }
  ...
  onEditBuilding(): void {
    if (!this.buildingId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-building`,
      this.buildingId,
    ]);
  }

  async onDeleteBuilding(): Promise<void> {
    if (!this.buildingId || !this.buildingData) return;

    const result = await Swal.fire({
      title: 'Delete Building?',
      text: `Are you sure you want to delete "${this.buildingData.name}" building? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.buildingService.softDeleteBuilding(this.buildingId).subscribe({
        next: () => {
          Swal.fire('Deleted!', 'Building has been deleted.', 'success');
          this.router.navigate([
            `/${this.currentUserRole.toLowerCase()}/buildings`,
          ]);
        },
        error: (err) => {
