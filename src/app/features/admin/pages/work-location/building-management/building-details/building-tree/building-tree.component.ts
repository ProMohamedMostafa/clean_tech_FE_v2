...
export class BuildingTreeComponent {
  ...
  ngOnInit(): void {
    if (this.buildingId) {
      this.loadCityTree(this.buildingId);
    }
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['buildingId'] && changes['buildingId'].currentValue) {
      this.loadCityTree(changes['buildingId'].currentValue);
    }
  }
  ...
  private loadCityTree(buildingId: number): void {
    this.treeLoaded = false;
    this.buildingService.getBuildingTreeById(buildingId).subscribe({
      next: (res) => {
        if (res?.data) {
