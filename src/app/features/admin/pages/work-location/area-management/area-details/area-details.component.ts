...
// ==================== CUSTOM COMPONENTS ====================
import { LocationDetailsInfoComponent } from './location-details-info/location-details-info.component';
...
@Component({
  selector: 'area-location-details',
  standalone: true,
  imports: [
    CommonModule,
    TranslateModule,
...
export class AreaDetailsComponent {
  ...
  areaData: AreaDetails | null = null;
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.areaId = id ? +id : null;
      if (this.areaId) {
        this.loadAreaDetails(this.areaId);
      } else {
  ...
  loadAreaDetails(id: number): void {
    this.isLoading = true;
    this.areaService.getAreaById(id).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load area details. Please try again later.'
          );
        }
      },
    });
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this area');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this area');
      },
    });
  }
  ...
  selectTab(tab: string): void {
    ...
    if (tab === 'users' && this.areaId) {
      this.loadUsersByArea(this.areaId);
    }
    if (tab === 'tasks' && this.areaId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.areaId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.areaId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.areaId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `area_${this.areaId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(`Area Report: ${this.areaData?.name || 'Unknown'}`, 105, yPos, {
      align: 'center',
    });
    yPos += 15;
    ...
    doc.save(`area_${this.areaId}_report.pdf`);
  }
  ...
  onEditArea(): void {
    if (!this.areaId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-area`,
      this.areaId,
    ]);
  }

  async onDeleteArea(): Promise<void> {
    if (!this.areaId || !this.areaData) return;

    const result = await Swal.fire({
      title: 'Delete Area?',
      text: `Are you sure you want to delete "${this.areaData.name}" area? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.areaService.deleteArea(this.areaId).subscribe({
        next: () => {
          ...
          this.router.navigate([
            `/${this.currentUserRole.toLowerCase()}/locations`,
          ]);
        },
        error: (err) => {
