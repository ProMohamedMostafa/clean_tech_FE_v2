...
export class OrganizationTreeComponent {
  ...
  ngOnInit(): void {
    if (this.organizationId) {
      this.loadCityTree(this.organizationId);
    }
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['organizationId'] && changes['organizationId'].currentValue) {
      this.loadCityTree(changes['organizationId'].currentValue);
    }
  }
  ...
  private loadCityTree(organizationId: number): void {
    this.treeLoaded = false;
    this.organizationService.getOrganizationTreeById(organizationId).subscribe({
      next: (res) => {
        if (res?.data) {
