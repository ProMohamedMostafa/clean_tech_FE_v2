...
@Component({
  selector: 'app-floor-tree',
  imports: [FontAwesomeModule, FormsModule, CommonModule],
  templateUrl: './floor-tree.component.html',
  styleUrl:'../../../area-management/area-details/location-tree/location-tree.component.scss',
})
export class FloorTreeComponent {
  ...
  ngOnInit(): void {
    if (this.floorId) {
      this.loadCityTree(this.floorId);
    }
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['floorId'] && changes['floorId'].currentValue) {
      this.loadCityTree(changes['floorId'].currentValue);
    }
  }
  ...
  private loadCityTree(floorId: number): void {
    this.treeLoaded = false;
    this.floorService.getFloorTreeById(floorId).subscribe({
      next: (res) => {
        if (res?.data) {
  ...
  private buildTree(data: any): TreeNode {
    return {
      id: data.id,
      name: data.name,
      // The response contains sections array directly, no floors inside floors
      sections:
