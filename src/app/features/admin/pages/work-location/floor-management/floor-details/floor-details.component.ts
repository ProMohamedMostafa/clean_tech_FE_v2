...
import { FloorDetailsInfoComponent } from './floor-details-info/floor-details-info.component';
import { FloorTreeComponent } from './floor-tree/floor-tree.component';
...
export class FloorDetailsComponent {
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.floorId = id ? +id : null;
      if (this.floorId) {
        this.loadFloorDetails(this.floorId);
      } else {
    ...
    this.floorService.getFloorById(id).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load floor details. Please try again later.'
          );
        }
      },
    });
  ...
  loadUsersByFloor(floorId: number): void {
    this.isLoading = true;
    const filters = {
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this floor');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this floor');
      },
    });
  }
  ...
  selectTab(tab: string): void {
    ...
    if (tab === 'users' && this.floorId) {
      this.loadUsersByFloor(this.floorId);
    }
    if (tab === 'tasks' && this.floorId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.floorId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.floorId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.floorId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `floor_${this.floorId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(`Floor Report: ${this.floorData?.name || 'Unknown'}`, 105, yPos, {
      align: 'center',
    });
    yPos += 15;
    ...
    doc.save(`floor_${this.floorId}_report.pdf`);
  }
  ...
  onEditFloor(): void {
    if (!this.floorId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-floor`,
      this.floorId,
    ]);
  }

  async onDeleteFloor(): Promise<void> {
    if (!this.floorId || !this.floorData) return;

    const result = await Swal.fire({
      title: 'Delete Floor?',
      text: `Are you sure you want to delete "${this.floorData.name}" floor? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.floorService.softDeleteFloor(this.floorId).subscribe({
        next: () => {
          Swal.fire('Deleted!', 'Floor has been deleted.', 'success');
          this.router.navigate([
            `/${this.currentUserRole.toLowerCase()}/floors`,
          ]);
        },
        error: (err) => {
