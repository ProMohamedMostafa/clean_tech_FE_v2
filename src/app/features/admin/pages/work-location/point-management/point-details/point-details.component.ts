...
import { PointDetailsInfoComponent } from './point-details-info/point-details-info.component';
import { PointQuestionsComponent } from "./point-questions/point-questions.component";
...
@Component({
  ...
  imports: [
    CommonModule,
    TranslateModule,
    PointDetailsInfoComponent,
    TableDataComponent,
    TaskContainerComponent,
    FormsModule,
    PointQuestionsComponent
],
...
export class PointDetailsComponent {
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.pointId = id ? +id : null;
      if (this.pointId) {
        this.loadPointDetails(this.pointId);
        this.loadUsersByPoint(this.pointId);
      } else {
  ...
  loadPointDetails(id: number): void {
    this.isLoading = true;
    this.pointService.getPointById(id).subscribe({
      next: (response: any) => {
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load point details. Please try again later.'
          );
        }
      },
    });
  ...
  loadUsersByPoint(pointId: number): void {
    this.isLoading = true;
    const filters = {
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this point');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this point');
      },
    });
  }
  ...
  selectTab(tab: string): void {
    ...
    if (tab === 'users' && this.pointId) {
      this.loadUsersByPoint(this.pointId);
    }
    if (tab === 'tasks' && this.pointId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.pointId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.pointId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.pointId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `point_${this.pointId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(`Point Report: ${this.pointData?.name || 'Unknown'}`, 105, yPos, {
      align: 'center',
    });
    yPos += 15;
    ...
    doc.save(`point_${this.pointId}_report.pdf`);
  }
  ...
  onEditPoint(): void {
    if (!this.pointId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-point`,
      this.pointId,
    ]);
  }

  async onDeletePoint(): Promise<void> {
    if (!this.pointId || !this.pointData) return;

    const result = await Swal.fire({
      title: 'Delete Point?',
      text: `Are you sure you want to delete "${this.pointData.name}" point? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.pointService.softDeletePoint(this.pointId).subscribe({
        next: () => {
          Swal.fire('Deleted!', 'Point has been deleted.', 'success');
          this.router.navigate([
            `/${this.currentUserRole.toLowerCase()}/points`,
          ]);
        },
        error: (err) => {
