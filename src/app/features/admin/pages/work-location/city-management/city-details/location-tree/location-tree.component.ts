...
export class LocationTreeComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit(): void {
    if (this.cityId) {
      this.loadCityTree(this.cityId);
    }
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['cityId'] && changes['cityId'].currentValue) {
      this.loadCityTree(changes['cityId'].currentValue);
    }
  }
  ...
  private loadCityTree(cityId: number): void {
    this.treeLoaded = false;
    this.cityService.getCustomCityTree(cityId).subscribe({
      next: (res) => {
        if (res?.data) {
        ...
        data.organizations?.map((org: any) => ({
          ...
          buildings:
            org.buildings?.map((building: any) => ({
              id: building.id,
              name: building.name,
              previousName: building.previousName,
              floors:
                building.floors?.map((floor: any) => ({
                  id: floor.id,
                  name: floor.name,
                  previousName: floor.previousName,
                  sections:
                    floor.sections?.map((section: any) => ({
                      id: section.id,
                      name: section.name,
                      previousName: section.previousName,
                      points:
                        section.points?.map((point: any) => ({
                          id: point.id,
                          name: point.name,
                          previousName: point.previousName,
                          device: point.device
                            ? {
                                id: point.device.id,
