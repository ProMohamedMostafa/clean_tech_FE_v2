...
export class CityDetailsComponent {
  ...
  private initializeRouteParams(): void {
    this.route.paramMap.subscribe((params) => {
      ...
      this.cityId = id ? +id : null;
      if (this.cityId) {
        this.loadCityDetails(this.cityId);
      } else {
    ...
    this.cityService.getCityById(id).subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403 || err.status === 401) {
          ...
          this.showError(
            'Could not load city details. Please try again later.'
          );
        }
      },
    });
  ...
  loadUsersByCity(cityId: number): void {
    this.isLoading = true;
    const filters = {
    ...
    this.userService.getUsersWithPagination(filters).subscribe({
      next: (response) => {
        ...
        if (response.succeeded) {
          ...
          this.showError('Failed to load users for this city');
        }
      },
      error: (err) => {
        this.isLoading = false;
        this.showError('Error loading users for this city');
      },
    });
  }
  ...
  selectTab(tab: string): void {
    ...
    if (tab === 'users' && this.cityId) {
      this.loadUsersByCity(this.cityId);
    }
    if (tab === 'tasks' && this.cityId) {
      this.loadTasks();
    }
    if (tab === 'shifts' && this.cityId) {
      this.loadShifts();
    }
    if (tab === 'attendance' && this.cityId) {
      this.loadAttendance();
    }
    if (tab === 'leaves' && this.cityId) {
      this.loadLeaves();
    }
  }
  ...
  private exportToExcel(): void {
    ...
    XLSX.writeFile(workbook, `city_${this.cityId}_data.xlsx`);
  }
  ...
  private exportToPDF(): void {
    ...
    doc.setFontSize(16);
    doc.text(`City Report: ${this.cityData?.name || 'Unknown'}`, 105, yPos, {
      align: 'center',
    });
    yPos += 15;
    ...
    doc.save(`city_${this.cityId}_report.pdf`);
  }
  ...
  onEditCity(): void {
    if (!this.cityId) return;
    this.router.navigate([
      `/${this.currentUserRole.toLowerCase()}/edit-city`,
      this.cityId,
    ]);
  }

  async onDeleteCity(): Promise<void> {
    if (!this.cityId || !this.cityData) return;

    const result = await Swal.fire({
      title: 'Delete City?',
      text: `Are you sure you want to delete "${this.cityData.name}" city? This will also delete all associated data.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: 'Delete',
      cancelButtonText: 'Cancel',
    ...
    if (result.isConfirmed) {
      this.cityService.softDeleteCity(this.cityId).subscribe({
        next: () => {
